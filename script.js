...
//const tlpPage = document.querySelector('#tlp-page');
...
function displayItems() {
  ...
  let html = '';
  if(arr.length == 0) {
    tbody.textContent = '<p>THERE IS NO OPEN ADD</p>';
  }
  arr.forEach((element, index) => {
    ...
    <td><button onClick="concession(${index})">Concession</button></td>
    <td><button onClick="remove(${index})" class="delete">Close</button></td>
    </tr>
    `
  });
